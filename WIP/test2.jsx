// Shape Layer Particle System with Turbulence, SP-ctrl layer, looping wiggle, and shape options

(function() {
    // Configures sliders with min/max values, default value, and callbacks
    function configureSlider(slider, min, max, defaultValue, textField, updateCallback, isFloat, constraints) {
        slider.value = defaultValue;
        slider.minvalue = min;
        slider.maxvalue = max;
        
        if (textField) {
            textField.text = isFloat ? defaultValue.toFixed(1) : Math.round(defaultValue);
        }
        
        slider.onChanging = function() {
            var value = slider.value;
            if (textField) {
                textField.text = isFloat ? value.toFixed(1) : Math.round(value);
            }
        ...
        if (textField) {
            textField.onChange = function() {
                var value = isFloat ? parseFloat(this.text) : parseInt(this.text);
                var minVal = constraints && constraints.min !== undefined ? constraints.min : min;
                var maxVal = constraints && constraints.max !== undefined ? constraints.max : max;
                
                if (!isNaN(value) && value >= minVal && value <= maxVal) {
                    slider.value = value;
                } else {
                    this.text = isFloat ? slider.value.toFixed(1) : Math.round(slider.value);
                }
    ...
    // Validate project and composition
    if (!app.project) {
    ...
    // Create panel for persistent usage
    ...
    tabbedPanel.preferredSize = [400, 350];
    ...
    // Configure slider using helper function
    configureSlider(countSlider, 1, 200, particleCount, countText, handleLiveUpdate, false, { min: 1 });
    ...
    configureSlider(velocitySlider, 0, 500, 100, velocityText, handleLiveUpdate);
    ...
    configureSlider(velocityVarSlider, 0, 200, 50, velocityVarText, handleLiveUpdate);
    ...
    configureSlider(velocityDirSlider, -180, 180, 0, velocityDirText, handleLiveUpdate);
    ...
    configureSlider(velocityDirVarSlider, 0, 180, 10, velocityDirVarText, handleLiveUpdate);
    ...
    configureSlider(gravitySlider, 0, 500, 98, gravityText, handleLiveUpdate);
    ...
    configureSlider(gravityDirSlider, -180, 180, 180, gravityDirText, handleLiveUpdate);
    ...
    configureSlider(resistanceSlider, 0, 100, 0, resistanceText, handleLiveUpdate);
    ...
    configureSlider(lifetimeSlider, 0.1, 20, 4, lifetimeText, handleLiveUpdate, true, { min: 0.1 });
    ...
    configureSlider(emitterWidthSlider, 0, 500, 0, emitterWidthText, handleLiveUpdate);
    ...
    configureSlider(emitterHeightSlider, 0, 500, 0, emitterHeightText, handleLiveUpdate);
    ...
    function addParticleControls(layer, maxParticles, shapeType) {
        ...
        if (shapeType === "Rectangle") {
            ...
            rectWidthRatio.property("Slider").setValue(loadedSettings && loadedSettings["Rectangle Width Ratio"] ? 
                loadedSettings["Rectangle Width Ratio"] : 1);
    ...
    configureSlider(particleSizeSlider, 1, 100, 10, particleSizeText, handleLiveUpdate, false, { min: 1 });
    ...
    configureSlider(sizeVarSlider, 0, 50, 5, sizeVarText, handleLiveUpdate);
    ...
    configureSlider(colorVarSlider, 0, 100, 0, colorVarText, handleLiveUpdate);
    ...
    configureSlider(colorOffsetSlider, 0, 100, 20, colorOffsetText, handleLiveUpdate);
    ...
    configureSlider(maxOpacitySlider, 0, 100, 100, maxOpacityText, handleLiveUpdate);
    ...
    configureSlider(opacityVarSlider, 0, 100, 20, opacityVarText, handleLiveUpdate);
    ...
    configureSlider(fadeInSlider, 0, 100, 10, fadeInText, handleLiveUpdate);
    ...
    configureSlider(fadeOutSlider, 0, 100, 80, fadeOutText, handleLiveUpdate);
    ...
    configureSlider(rotSpeedSlider, -360, 360, 0, rotSpeedText, handleLiveUpdate);
