// Color Manager
// Opens the Color Manager panel directly when run
// Click: Open Color Manager UI
...
(function() {
    ...
    // (Removed background cycling utilities to slim the script)
    ...
    function createSingleColorTab(parent) {
        ...
        var hsbText = previewGroup.add("statictext", undefined, "HSB: 0°,0,100");
        hsbText.preferredSize.width = 140;
        hsbText.alignment = "left";
        hsbText.graphics.font = FontUtils.getSmallFont();
        ...
        // HSB sliders
        var hGroup = controlsGroup.add("group");
        hGroup.orientation = "row";
        hGroup.spacing = 4;
        var hLabel = hGroup.add("statictext", undefined, "H:");
        hLabel.preferredSize.width = 15;
        var hSlider = hGroup.add("slider", undefined, 0, 0, 360);
        hSlider.preferredSize.width = 160;
        var hValue = hGroup.add("edittext", undefined, "0");
        hValue.characters = 3;

        var sGroup = controlsGroup.add("group");
        sGroup.orientation = "row";
        sGroup.spacing = 4;
        var sLabel = sGroup.add("statictext", undefined, "S:");
        sLabel.preferredSize.width = 15;
        var sSlider = sGroup.add("slider", undefined, 0, 0, 100);
        sSlider.preferredSize.width = 160;
        var sValue = sGroup.add("edittext", undefined, "0");
        sValue.characters = 3;
        ...
        var bSlider = bGroup.add("slider", undefined, 100, 0, 100);
        bSlider.preferredSize.width = 160;
        var bValue = bGroup.add("edittext", undefined, "100");
        bValue.characters = 3;
        ...
        // Function to update color preview from HSB
        function updateColorPreview() {
            var h = parseFloat(hSlider.value) || 0;
            var s = (parseFloat(sSlider.value) || 0) / 100;
            var v = (parseFloat(bSlider.value) || 0) / 100;
            var rgb = ColorUtils.hsvToRgb(h, s, v);
            colorPreview.currentColor = rgb;
            hsbText.text = "HSB: " + Math.round(h) + "°," + Math.round(s * 100) + "," + Math.round(v * 100);
        ...
        // Function to update HSB sliders from hex input
        function updateFromHex() {
            var rgb = ColorUtils.hexToRgb(hexInput.text);
            if (rgb) {
                var hsv = ColorUtils.rgbToHsv(rgb);
                hSlider.value = Math.round(hsv[0]);
                sSlider.value = Math.round(hsv[1] * 100);
                bSlider.value = Math.round(hsv[2] * 100);
                hValue.text = Math.round(hsv[0]);
                sValue.text = Math.round(hsv[1] * 100);
                bValue.text = Math.round(hsv[2] * 100);
                updateColorPreview();
            }
        }
        ...
        hSlider.onChanging = function() { 
            hValue.text = Math.round(this.value); 
            updateColorPreview();
        };
        sSlider.onChanging = function() { 
            sValue.text = Math.round(this.value); 
            updateColorPreview();
        };
        ...
        // Initialize
        // Initialize from default preview color (white)
        hSlider.value = 0; hValue.text = "0";
        sSlider.value = 0; sValue.text = "0";
        bSlider.value = 100; bValue.text = "100";
        updateColorPreview();
    ...
    function parseColorsFromText(text) {
        ...
        // 4. CSS cmyk() support
        var cmykRegex = /cmyk\s*\(\s*(\d{1,3})%?\s*,\s*(\d{1,3})%?\s*,\s*(\d{1,3})%?\s*,\s*(\d{1,3})%?\s*\)/gi;
        while ((match = cmykRegex.exec(text)) !== null) {
            var c = parseInt(match[1], 10) / 100;
            var m = parseInt(match[2], 10) / 100;
            var y = parseInt(match[3], 10) / 100;
            var k = parseInt(match[4], 10) / 100;
            var r = (1 - c) * (1 - k);
            var g = (1 - m) * (1 - k);
            var b = (1 - y) * (1 - k);
            colors.push([r, g, b]);
        ...
        // 5. Pantone support (requires mapping)
        var pantoneRegex = /PANTONE\s+([0-9A-Za-z\s\-\.]+)/gi;
        var pantoneMap = {
            "186 C": "#E4002B",
            "300 U": "#005EB8"
            // Add more Pantone code to hex mappings as needed
        };
        while ((match = pantoneRegex.exec(text)) !== null) {
            var code = match[1].trim();
            if (pantoneMap[code]) {
                var rgb = ColorUtils.hexToRgb(pantoneMap[code]);
    ...
    // Always open the Color Manager UI when the script runs
    buildUI();
    state.panel.center();
    state.panel.show();
})();