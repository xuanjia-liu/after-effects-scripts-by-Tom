...
(function() {
    ...
    // Add custom shape group
    var customShapeGroup = basicTab.add("group");
    customShapeGroup.orientation = "row";
    customShapeGroup.alignChildren = ["left", "center"];
    customShapeGroup.add("statictext", undefined, "Select Shape:");
    var customShapeButton = customShapeGroup.add("button", undefined, "Pick Layer");
    var customShapeText = customShapeGroup.add("statictext", undefined, "No layer selected");
    customShapeText.preferredSize.width = 150;
    customShapeGroup.visible = false; // Hide initially
    
    // Store reference to the selected shape layer
    var selectedCustomShape = null;
    
    // Pick custom shape layer button click handler
    customShapeButton.onClick = function() {
        if (app.project.activeItem && app.project.activeItem instanceof CompItem) {
            var selectedLayers = app.project.activeItem.selectedLayers;
            if (selectedLayers.length > 0) {
                var layer = selectedLayers[0];
                if (layer.matchName === "ADBE Vector Layer" || 
                    (layer.property("Contents") && layer.property("Contents").numProperties > 0)) {
                    selectedCustomShape = layer;
                    customShapeText.text = layer.name;
                } else {
                    alert("Please select a shape layer");
                }
            } else {
                alert("Please select a shape layer in the composition");
            }
        } else {
    ...
    shapeDropdown.onChange = function() {
        ...
        customShapeGroup.visible = (shapeDropdown.selection.text === "Custom");
        handleLiveUpdate();
    };
    ...
    function handleLiveUpdate() {
        if (liveUpdateCheckbox.value && systemDropdown.selection.index > 0) {
            ...
            try {
                ...
                } else if (selectedSystem.name.indexOf("[Custom]") !== -1) {
                            ...
                            if (userParticleCount > currentParticleCount) {
                                ...
                                for (var k = currentParticleCount + 1; k <= userParticleCount; k++) {
                                    ...
                                    if (currentShapeType === "Circle") {
                                        ...
                                        // For Custom shapes, we need to find the original custom shape to duplicate
                                        try {
                                            // Add a path shape
                                            shape = particleGroup.property("Contents").addProperty("ADBE Vector Shape - Group");
                                            shape.name = "Custom Shape";
                                            
                                            // Look for the first existing particle with a custom shape
                                            var sourcePath = null;
                                            
                                            // Search existing particles for a custom shape path to copy
                                            for (var m = 1; m <= contents.numProperties && !sourcePath; m++) {
                                                var existingParticle = contents.property(m);
                                                if (existingParticle.name.indexOf("Particle ") === 0) {
                                                    var existingContents = existingParticle.property("Contents");
                                                    
                                                    for (var n = 1; n <= existingContents.numProperties; n++) {
                                                        var existingContent = existingContents.property(n);
                                                        if (existingContent.matchName === "ADBE Vector Shape - Group" && 
                                                            existingContent.name === "Custom Shape") {
                                                            // Found an existing custom shape to copy
                                                            var path = existingContent.property("ADBE Vector Shape");
                                                            if (path) {
                                                                sourcePath = path;
                                                                break;
                                                            }
                                            ...
                                            if (sourcePath) {
                                                // Copy the path to our new shape
                                                var pathValue = sourcePath.value;
                                                shape.property("ADBE Vector Shape").setValue(pathValue);
                                                
                                                // Get original shape center coordinates if possible
                                                var originalCenter = [0, 0];
                                                try {
                                                    // Try to find the center of the shape from the source
                                                    var sourceGroup = selectedCustomShape.property("Contents").property(1);
                                                    if (sourceGroup && sourceGroup.property("Transform") && 
                                                        sourceGroup.property("Transform").property("Position")) {
                                                        originalCenter = sourceGroup.property("Transform").property("Position").value;
                                                    }
                                                } catch(err) {
                                                    // If we can't get the center, we'll use [0,0]
                                                }
                                                
                                                // First make sure the particle group's transform anchor point is set to negative of original position
                                                var groupTransform = particleGroup.property("Transform");
                                                if (groupTransform) {
                                                    if (groupTransform.property("Anchor Point")) {
                                                        groupTransform.property("Anchor Point").setValue([-originalCenter[0], -originalCenter[1]]);
                                                    }
                                                    
                                                    // Set position to origin
                                                    if (groupTransform.property("Position")) {
                                                        groupTransform.property("Position").setValue([0, 0]);
                                                    }
                                                }
                                                
                                                // Add scaling expression for the path that also centers the shape
                                                shape.property("ADBE Vector Shape").expression = 
                                                    "var ctrlLayer = thisComp.layer(\"SP-ctrl " + instanceNum + " [Custom]\");\n" +
                                                    "var particleSize = ctrlLayer.effect(\"Particle Size\")(\"Slider\");\n" +
                                                    "var originalPath = value;\n\n" +
                                                    
                                                    "// Generate a seed for this particle\n" +
                                                    "seedRandom(" + i + " + 5555, true);\n" +
                                                    "var offset = random(0, 100);\n\n" +
                                                    
                                                    "// Calculate scale factor (convert from the default size of 10 to the particle size)\n" +
                                                    "var scaleFactor = particleSize / 10;\n\n" +
                                                    ...
                                                    "try {\n" +
                                                    "    // For Shape paths\n" +
                                                    "    if (typeof originalPath.points === 'function') {\n" +
                                                    "        var vertices = originalPath.points();\n" +
                                                    "        var inTangents = originalPath.inTangents();\n" +
                                                    "        var outTangents = originalPath.outTangents();\n" +
                                                    "        var closed = originalPath.isClosed();\n\n" +
                                                    "        \n" +
                                                    "        // First find the center of all vertices to center the shape\n" +
                                                    "        var centerX = 0;\n" +
                                                    "        var centerY = 0;\n" +
                                                    ...
                                                    "        \n" +
                                                    "        // Scale each point and tangent around the center\n" +
                                                    "        for (var i = 0; i < vertices.length; i++) {\n" +
                                                    "            // Center the vertex by subtracting the center, then scale\n" +
                                                    "            vertices[i][0] = (vertices[i][0] - centerX) * scaleFactor;\n" +
                                                    "            vertices[i][1] = (vertices[i][1] - centerY) * scaleFactor;\n" +
                                                    "            inTangents[i] = inTangents[i] * scaleFactor;\n" +
                                                    "            outTangents[i] = outTangents[i] * scaleFactor;\n" +
                                                    "        }\n\n" +
                                                    "        \n" +
                                                    "        newPath.createPath(vertices, inTangents, outTangents, closed);\n" +
                                                    "    } else {\n" +
                                                    "        // For Shape objects that don't have these functions\n" +
                                                    "        // Use the original path and apply a scale transform instead\n" +
                                                    "        newPath = originalPath;\n" +
                                                    "        var pathScale = content('Transform').scale;\n" +
                                                    "        pathScale[0] = pathScale[0] * scaleFactor;\n" +
                                                    "        pathScale[1] = pathScale[1] * scaleFactor;\n" +
                                                    "        \n" +
                                                    "        // Try to center the path if possible\n" +
                                                    "        try {\n" +
                                                    "            var pathAP = content('Transform').anchorPoint;\n" +
                                                    "            var pathPos = content('Transform').position;\n" +
                                                    "            // Set anchor point to origin\n" +
                                                    "            pathAP.setValue([0, 0]);\n" +
                                                    "            // Set position to origin\n" +
                                                    "            pathPos.setValue([0, 0]);\n" +
                                                    "        } catch(err) {\n" +
                                                    "            // Ignore error if transform properties are not available\n" +
                                                    "        }\n" +
                                                    "    }\n" +
                                                    "} catch(err) {\n" +
                                                    "    // If there's an error, just scale the entire shape\n" +
                                                    "    newPath = originalPath;\n" +
                                                    "}\n\n" +
                                                    
                                                    "newPath;";
                                                
                                                // Only set position to match anchor point if the flag is false
                                                if (!keepParticlePositionAtOrigin) {
                                                    // Ensure the shape group position matches the anchor point
                                                    // to maintain the visual center
                                                    var groupTransform = particleGroup.property("Transform");
                                                    if (groupTransform) {
                                                        if (groupTransform.property("Position") && groupTransform.property("Anchor Point")) {
                                                            var anchorPoint = groupTransform.property("Anchor Point").value;
                                                            groupTransform.property("Position").setValue(anchorPoint);
                                                        }
                                                    }
                                                }
                                            } else {
                                                // If no existing path found, use a circle as fallback
                                                shape.remove();
                                                shape = particleGroup.property("Contents").addProperty("ADBE Vector Shape - Ellipse");
                                                shape.name = "Fallback Circle";
                                                shape.property("ADBE Vector Ellipse Size").setValue([10, 10]);
                                            }
                                        } catch (err) {
                                            alert("Error creating custom shape: " + err.toString());
                                            // Use a fallback circle
    ...
    systemDropdown.onChange = function() {
        if (systemDropdown.selection.index > 0) {
            ...
            } else if (loadedSettings && loadedSettings.hasOwnProperty("isCustom")) {
                shapeDropdown.selection = 4; // Custom
                if (loadedSettings.hasOwnProperty("Custom Shape")) {
                    customShapeButton.text = "Pick Layer";
                    customShapeText.text = "Custom Shape";
                }
                polygonGroup.visible = false;
                starGroup.visible = false;
                rectangleGroup.visible = false;
    ...
    function resetControlsToDefaults() {
        ...
        // Reset custom shape selection
        customShapeText.text = "No layer selected";
    ...
    okButton.onClick = function() {
        ...
        if (!isNaN(userInput) && userInput > 0 && userInput <= 200) {
            ...
            if (shapeType === "Star") {
                ...
                // Store custom shape in settings
                if (!loadedSettings) loadedSettings = {};
                loadedSettings.isCustom = true;
                loadedSettings["Custom Shape"] = selectedCustomShape ? selectedCustomShape.name : "No custom shape selected";
            }
    ...
    // Add global variable to control particle position behavior
    var keepParticlePositionAtOrigin = true;
    ...
    function createParticleSystem() {
        try {
            ...
            var activeComp = app.project.activeItem;
            var shapeType = shapeDropdown.selection.text;
            var particleCount = parseInt(countText.text);
            
            app.beginUndoGroup("Create Particle System");
            
            // Check if we're in Custom shape mode but no shape is selected
            if (shapeType === "Custom" && !selectedCustomShape) {
                alert("Please select a shape layer first.");
                app.endUndoGroup();
                return;
            }

            // If using a custom shape, prepare it before creating particles
            var tempCustomShape = null;
            if (shapeType === "Custom" && selectedCustomShape) {
                // Find the first path in the custom shape and its group
                var sourceGroup = null;
                var sourcePath = null;
                
                try {
                    // Find the first vector group in the shape
                    var sourceContents = selectedCustomShape.property("Contents");
                    
                    // Recursive function to find the first path
                    function findFirstPathAndGroup(prop, parentGroup) {
                        if (!prop) return null;
                        ...
                        for (var j = 1; j <= prop.numProperties; j++) {
                            ...
                            if (subProp.matchName === "ADBE Vector Group") {
                                var groupContents = subProp.property("Contents");
                                if (groupContents) {
                                    for (var k = 1; k <= groupContents.numProperties; k++) {
                                        var shapeContent = groupContents.property(k);
                                        
                                        // Check if this is a path
                                        if (shapeContent.matchName === "ADBE Vector Shape - Group") {
                                            var path = shapeContent.property("ADBE Vector Shape");
                                            if (path) {
                                                return {
                                                    path: path,
                                                    group: subProp
                                ...
                                // Recursively check this group's contents
                                var result = findFirstPathAndGroup(subProp.property("Contents"), subProp);
                                if (result) return result;
                            }
                            // Or check if this is a direct path
                            else if (subProp.matchName === "ADBE Vector Shape - Group") {
                            ...
                            // Recursively check other group contents
                            else if (subProp.matchName === "ADBE Vector Contents" || 
                                    subProp.propertyType === PropertyType.INDEXED_GROUP) {
                                var result = findFirstPathAndGroup(subProp, parentGroup);
                                if (result) return result;
                            }
                        }
                        return null;
                    ...
                    // Find the first path and its group in the shape
                    var result = findFirstPathAndGroup(sourceContents, null);
                    if (result) {
                        sourcePath = result.path;
                        sourceGroup = result.group;
                    }
                } catch(err) {
                    alert("Error analyzing custom shape: " + err.toString());
                }
            }
            
            // Track existing instances to create a unique name
            var instanceNum = 1;
            var existingSystems = findExistingParticleSystems(activeComp);
            if (existingSystems.length > 0) {
                // Extract numbers from existing system names to find the next available
                var maxInstanceNum = 0;
                for (var i = 0; i < existingSystems.length; i++) {
                    var match = existingSystems[i].name.match(/\d+/);
                    if (match) {
                        var num = parseInt(match[0]);
                        if (num > maxInstanceNum) maxInstanceNum = num;
                    }
                }
                instanceNum = maxInstanceNum + 1;
            }
            
            // Create the control layer
            var controlLayer = createControlLayer(activeComp, instanceNum);
            
            // Update control layer name based on shape type
            if (shapeType === "Star") {
                ...
                controlLayer.name = "SP-ctrl " + instanceNum + " [Custom]";
                // Add a comment to remember which shape layer was used
                if (selectedCustomShape) {
                    controlLayer.comment = "Based on shape: " + selectedCustomShape.name;
                }
            }
            
            // Add effect controls based on the selected shape type
            ...
            // Create the particles based on the shape type
...
sliderProperty.maximum = 32;
            ...
            // Add custom shape specific controls
            var customShapeMorph = layer.Effects.addProperty("ADBE Slider Control");
            customShapeMorph.name = "Custom Shape Morph";
            customShapeMorph.property("Slider").setValue(0); // Default no morph effect
            var sliderProperty = customShapeMorph.property("Slider");
            sliderProperty.minimum = 0;
            sliderProperty.maximum = 100;
            
            var customShapeRotation = layer.Effects.addProperty("ADBE Angle Control");
            customShapeRotation.name = "Custom Shape Rotation";
            customShapeRotation.property("Angle").setValue(0); // Default no rotation
            
            var customShapeScale = layer.Effects.addProperty("ADBE Slider Control");
            customShapeScale.name = "Custom Shape Scale";
            customShapeScale.property("Slider").setValue(100); // Default 100%
            var sliderProperty = customShapeScale.property("Slider");
            sliderProperty.minimum = 10;
            sliderProperty.maximum = 500;
        }
    ...
    function createParticles(layer, instanceNum, count, shapeType) {
        ...
        for (var i = 1; i <= count; i++) {
            ...
            else if (shapeType === "Custom" && selectedCustomShape) {
                try {
                    // Add a path shape
                    shape = particleGroup.property("Contents").addProperty("ADBE Vector Shape - Group");
                    shape.name = "Custom Shape";
                    
                    // Find first path in the custom shape layer
                    var sourcePath = null;
                    var sourceContents = selectedCustomShape.property("Contents");
                    
                    // Function to recursively find the first path in the shape layer
                    function findFirstPath(prop) {
                        if (!prop) return null;
                        ...
                        for (var j = 1; j <= prop.numProperties; j++) {
                            ...
                            // Recursively check group contents
                                     ...
                                     subProp.propertyType === PropertyType.INDEXED_GROUP) {
                                var result = findFirstPath(subProp);
                                if (result) return result;
                            }
                        }
                        return null;
                    ...
                    // Find a path in the custom shape
                    sourcePath = findFirstPath(sourceContents);
                    ...
                    if (sourcePath) {
                        ...
                        // Use standard position and anchor point at origin
                        var groupTransform = particleGroup.property("Transform");
                        if (groupTransform) {
                            // Set anchor point to origin
                            if (groupTransform.property("Anchor Point")) {
                                groupTransform.property("Anchor Point").setValue([0, 0]);
                            }
                            // Set position to origin
                            if (groupTransform.property("Position")) {
                                groupTransform.property("Position").setValue([0, 0]);
                        ...
                        // Add scaling expression for the path that centers the shape
                        shape.property("ADBE Vector Shape").expression = 
                            "var ctrlLayer = thisComp.layer(\"SP-ctrl " + instanceNum + " [Custom]\");\n" +
                            "var particleSize = ctrlLayer.effect(\"Particle Size\")(\"Slider\");\n" +
                            "var originalPath = value;\n\n" +
                        ...
                        // If no path found, use a circle as fallback
                        shape.remove();
                        shape = particleGroup.property("Contents").addProperty("ADBE Vector Shape - Ellipse");
                        shape.name = "Fallback Circle";
                        shape.property("ADBE Vector Ellipse Size").setValue([10, 10]);
                    ...
                    alert("Error creating custom shape: " + err.toString());
                    // Use a fallback circle if custom shape fails
                    if (shape) shape.remove();
                    shape = particleGroup.property("Contents").addProperty("ADBE Vector Shape - Ellipse");
                    shape.name = "Fallback Circle";
                    shape.property("ADBE Vector Ellipse Size").setValue([10, 10]);
